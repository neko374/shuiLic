<template>
  <div class="app-container">
    <TableTe v-model="list" :td-width="[8,8,8]" class="tablete1" />
    ...
    <div class="botBox" :style="{height:botBoxHeight}">
      ...
      <div class="photovoltaic" style="height:calc(100% - 50px);width:98%;margin-left:2%">
        <charts1 v-model="chartName" ref="chart1"/>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    if (this.$store.state.pvOperate.pv1Cur) { this.pv1 = this.$store.state.pvOperate.pv1Cur }
    if (this.$store.state.pvOperate.pv1Chart) { this.chartName1 = this.$store.state.pvOperate.pv1Chart }


    this.list = this.pv1
    this.chartName = this.chartName1
    this.list[0][3].stateQk.forEach(value => {
      value.state = 0
    })
    this.reload()
    this.reloadC()
  ...
  methods: {
    ...
    fzList(name, val) {
      for (const j in this.list) {
        for (const i in this.list[j]) {
          if (this.list[j][i].name === name) {
            this.list[j][i].value = val
            break
          }
        }
      }
