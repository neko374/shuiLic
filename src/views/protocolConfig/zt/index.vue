<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="BIT" width="62">
        <template slot-scope="{row}">
...
import { mqttgybitZtList, mqttgybitZtOnlyList, mqttgybitZtSave, mqttgybitZtUpdate, mqttgybitZtDelete, mqttgybitZtBatchSave, mqttgybitZtBatchUpdate } from '@/api/mqtt/gybitZt'
...
export default {
  ...
  methods: {
    getList() {
      ...
      mqttgybitZtOnlyList(data).then(response => {
        if (response.code === 0) {
    ...
    objSelectChange(val, disable) {
      ...
      mqttgyycyxList(data).then(response => {
        ...
        mqttgyycyxList(data).then(response => {
          ...
          this.registerSelectOptionsDia = res.filter(v => /^ZT/.test(v.pidType)).map(item => { return { value: item.pidType, label: item.comment } })
        })
      })
    },
    handleCreate() {
    ...
    handleDelete(row) {
      this.$confirm('此操作将永久删除该状态, 是否继续?', '提示', {
        ...
        mqttgybitZtList({}).then(response => {
          mqttgybitZtList({ limit: response.page.totalCount.toString(), gyNameId: row.gyNameId, pidType: row.pidType }).then(response => {
            const res = response.page.list
            const data = res.map(item => item.id)
            mqttgybitZtDelete(data).then(response => {
              if (response.code === 0) {
          ...
          mqttgybitZtList(data).then(response=>{
            if(response.code==0) {
              ...
              if(resz.length==0) {
                ...
                data.bitType = 0
                data.comment = ''
                mqttgybitZtSave(data).then(response => {
                  if (response.code === 0) {
    ...
    updateData({ ...data }) {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          mqttgybitZtUpdate(data).then(response => {
            if (response.code === 0) {
              this.dialogFormVisible = false
              this.InitOptiList = true
              this.getList()
    ...
    importFileDemo(obj) {
      ...
      reader.onload = e => {
        ...
        mqttgybitZtList({ limit: '1', gyNameId: gynameid }).then(response => {
          if (response.code !== 0) {
            this.$notify({
              title: 'Failed',
              message: response.msg,
          ...
          mqttgybitZtList({ limit: response.page.totalCount.toString(), gyNameId: gynameid }).then(response => {
            if (response.code === 0) {
              ...
              console.log(111)
              if (listc.length !== 0) {
                mqttgybitZtBatchSave(listc).then(response => {
                  if (response.code === 0) {
                    this.currentPage = 1
                    this.getList()
                    this.$notify({
              ...
              if (listu.length !== 0) {
                mqttgybitZtBatchUpdate(listu).then(response => {
                  if (response.code === 0) {
                    this.currentPage = 1
                    this.getList()
                    this.$notify({
