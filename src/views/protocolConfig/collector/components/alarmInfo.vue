  ...
  <div>
    ...
    >
      ...
      <el-table-column label="操作" width="60">
        <template slot-scope="{row}">
          <el-button size="mini" class="TableButtonView" @click="$router.push({path:'/protocolConfig/alarm',query:{gyid:row.gyNameId+row.gyObjId,xyid:row.pidType}})">
            查看
          </el-button>
        </template>
      </el-table-column>
    ...
    <dialogAlarm v-model="dialogztvalue" :abc.sync="innerdialogFormVisibleBit" />
  </div>
</template>
...
import dialogAlarm from './dialogAlarm'
