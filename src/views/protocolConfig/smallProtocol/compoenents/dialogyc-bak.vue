<template>
  <div>
    <el-dialog :title="textMap[text] + '('+value.name+')'" :visible.sync="dialogFormVisible" width="88%" height="90%" class="ycdialog">
      ...
      <el-radio-group v-model="tabPosition" size="medium" style="margin-bottom: 10px;">
        <el-radio-button label="项目规约"></el-radio-button>
        <el-radio-button label="项目完整规约"></el-radio-button>
      </el-radio-group>
      <el-form ref="ycForm" v-show="tabPosition=='项目规约'" class="powerDiaTable" status-icon size="mini" inline-message :model="ycformmodel">
        <el-table
        ...
        >
          ...
          <el-table-column label="基础规约ID" min-width="80">
            <template slot-scope="{row}">
              <span>{{ row.slaveGyNameId }}</span>
            </template>
          </el-table-column>
          <el-table-column label="基础规约对象名称" min-width="110">
            <template slot-scope="{row}">
          ...
          <el-table-column label="基础规约规约起始PID" min-width="120">
            <template slot-scope="{row}">
      ...
      <el-form ref="ycForm" v-show="tabPosition=='项目完整规约'" class="powerDiaTable" status-icon size="mini" inline-message :model="ycformmodel">
        <el-table
          v-loading="listLoading"
          :data="list"
          max-height="400"
