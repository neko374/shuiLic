    ...
    >
      <el-form ref="ycaddform" size="mini" :rules="rules" :model="ycaddform[ycdialogstatus]" label-position="left" label-width="130px">
        ...
        <el-form-item label="协议名称" prop="pidType">
          <el-input v-model.number="ycaddform[ycdialogstatus].pidType" />
          <!-- <el-select
        ...
        { value: this.text, label: '遥测',
          children: [
            { value: 'e', label: '电气量', children: [
              ...
              { value: 'switch', label: '开关', children: [
                { value: 'warn', label: '告警' },
                { value: 'zt', label: '状态' }
              ] },
              { value: 'pcs', label: 'PCS', children: [
                { value: 'yc', label: '遥测' },
                { value: 'warn', label: '告警' },
                { value: 'zt', label: '状态' }
              ] },
              { value: 'bms', label: 'BMS', children: [
                { value: 'yc', label: '遥测' },
                { value: 'warn', label: '告警' },
                { value: 'zt', label: '状态' }
              ] }
    ...
    addYcInfoForm({ ...data }) {
      this.$refs['ycaddform'].validate((valid) => {
        if (valid) {
          ...
          data.dchg = 'F'
          data.type = this.pidTypeToType(data.pidType)
          ...
          mqttgyycyxList({ limit: response.page.totalCount.toString(), ...this.xztj }).then(response => {
            if (response.code === 0) {
              ...
              j_data.forEach((value, j) => {
                const isindex = false
                if (('畅洋点序号' in value) && ('协议名称' in value) && ('字节类型' in value) && ('描述' in value)) {
                  if (this.jyvalueType(value.字节类型, this.valueTypeSelectoptions) && value.畅洋点序号 !== undefined) {
                    if (j < list.length) {
                      const val = list[j]
                      listu.push({
                        id: val.id,
                        ...
                        msgType: this.text,
                        dchg: 'F',
                        type:this.pidTypeToType(value.协议名称)
                      })
