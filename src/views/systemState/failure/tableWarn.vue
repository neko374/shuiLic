<template>
  <div style="height:100%;padding:10px;" class="tableWarn">
    <div ref="tabWarn">
      ...
      <el-form size="mini" :inline="true" @keyup.enter.native="handleFilter">
        ...
        <el-form-item>
          ...
          >
            <el-option
              v-for="(item,index) in ztSelectOptions"
              :key="'zt'+index"
              :label="getztLabel(item)"
              :value="item.value"
            >
              <span style="float: left">{{ item.label }}</span>
              <span style="float: right; color: #8492a6; font-size: 13px">{{ item | xsPidType(pidTypeComment,'label') }}</span>
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
        ...
        <el-form-item>
          ...
          >
            <el-option
              v-for="item in ztStateSelectOptions"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
    ...
    >
      ...
      <el-table-column prop="farmProtocolName" label="故障点含义" min-width="120" show-overflow-tooltip>
        <template slot-scope="{row}">
      ...
      <el-table-column prop="comment" label="故障状态" min-width="80" show-overflow-tooltip>
        <template slot-scope="{row}">
          <span :style="{color:getgzstatecolor(row.comment)}">{{ row.comment }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="comment" label="故障内容" min-width="100" show-overflow-tooltip>
        <template slot-scope="{row}">
          <el-button size="mini" class="TableButtonView" @click="GZView(row)">查看</el-button>
        </template>
      </el-table-column>
      <el-table-column prop="comment" label="故障比较" min-width="100" show-overflow-tooltip>
        <template slot-scope="{row}">
    ...
    <el-dialog v-if="dialogStatus==='GZView'" align="center" :title="temp.GZView.type+'('+temp.GZView.farmProtocolName+')'" :visible.sync="dialogFormVisible" custom-class="warnSt">
      <el-table
      ...
      >
        ...
        <el-table-column prop="valComment" label="值含义" min-width="70" show-overflow-tooltip>
          <template slot-scope="{row}">
            <el-tag v-if="row.ztalarm" :type="tagType(row)">{{ row.valComment }}</el-tag>
            <span v-else>{{ row.valComment }}</span>
          </template>
        </el-table-column>
      </el-table>
    </el-dialog>

    <el-dialog v-if="dialogStatus==='GZCompare'" align="center" :title="temp.GZCompare.type+'('+temp.GZCompare.farmProtocolName+')'" :visible.sync="dialogFormVisible" custom-class="warnSt">
      <!-- <div slot="title" class="dialog-title">
        <span class="title-text">{{ temp.GZCompare | xsPidType1(pidTypeComment) }}</span>
      </div> -->
      <gzCompare
        :data="temp.GZCompare"
...
<script>
import { getYcePvWarn, getYcePvWarnDynamics, getYcePvWarnDynamicsList } from '@/api/mqtt/ycepvwarn'
import { mqttgyycyxList } from '@/api/mqtt/gyycyx'
import { mqttgybitAlarmList } from '@/api/mqtt/gybitAlarm'
import { mqttgybitAlarmvalList } from '@/api/mqtt/gybitAlarmval'
...
export default {
  ...
  data() {
    return {
      ...
      listTbaleHeader: [
        ...
        { value: 'farmProtocolName', label: '故障点含义' },
        { value: 'v1', label: '值' },
        { value: 'comment', label: '故障状态' },
      ...
      queryBasicInfo: {
        dbName: 'fsvpp',
        tableName: 'yc_e_battery_warn',
        projectId: '16'
        // snId:'004_0013_0007_02_SJZ0'
      },
      ztInfoList: null,
      ztValInfoList: null,
      alarmInfoList: null,
  ...
  async created() {
    ...
    if (this.$route.query.deviceName) {
      this.deviceSelectValue = this.$route.query.deviceName
      this.ztStateSelectValue = 1
      this.ztSelectValue = this.$route.query.pidType
      this.deviceTypeSelectValue = this.$route.query.gyObjId
      const devicetypetable = this.deviceTypeSelectOptions.filter(v=>v.value==this.deviceTypeSelectValue)
      this.queryBasicInfo.tableName = 'yc_e_'+devicetypetable[0].table+'_warn'
    }
    // this.getList()
    this.getOtherList()
    this.getListAll()
  ...
  methods: {
    ...
    getgzstatecolor(data) {
      if (data == '故障恢复' || data == undefined) { return '' }
      else { return 'red' }
    },
    // 表格内容
    getList() {
      ...
      if (this.ztStateSelectValue !== '') {
        data.ztState = this.ztStateSelectValue
        if (data.ztState == 1) { list = list.filter(v => v.comment != '故障恢复') } else { list = list.filter(v => v.comment == '故障恢复') }
      }
    ...
    selectChange(val) {
        ...
        if (this.ztSelectValue === '') {
          this.ztSelectOptions = list.reduce((pre, v) => {
            const resd = { value: v.type, label: v.type, snId: v.snId }
            const sn = v.snId.split('_')
            resd.gyNameId = sn[2] || ''
            resd.gyObjId = sn[0] || ''
            const sbg = pre.find(l => l.label == resd.label && l.gyNameId == resd.gyNameId && l.gyObjId == resd.gyObjId)
            if (!sbg) { pre.push(resd) }
            return pre
          }, [])
        }
        if (this.deviceSelectValue === '') {
    ...
    getListAll() {
      ...
      getYcePvWarnDynamicsList(data, 60 * 1000).then(response => {
        if (response.code === 0) {
          const res = response.list
          this.listAll = res
          .map(v=>{v.comment = (v.comment==''?'故障恢复':v.comment);return v})
          this.selectChange()
          this.getList()
    ...
    getAlarmList() {
      ...
      mqttgybitAlarmList(data1).then(response => {
        data1.limit = response.page.totalCount + ''
        mqttgybitAlarmList(data1).then(response => {
          const res = response.page.list
          this.alarmInfoList = res.reduce((pre, cur) => {
            pre.push({ ...cur })
            pre.push({ ...cur, pidType: ('ZT' + cur.pidType.slice(2)) })
            return pre
          }, [])
        }).catch(() => {
          this.alarmInfoList = []
      ...
      mqttgybitAlarmvalList(data2).then(response => {
        data2.limit = response.page.totalCount + ''
        mqttgybitAlarmvalList(data2).then(response => {
          const res = response.page.list
          this.alarmValInfoList = res
        }).catch(() => {
          this.alarmValInfoList = []
        })
      }).catch(() => {
        this.alarmValInfoList = []
      })
    ...
    getOtherList() {
      ...
      //         // this.ztSelectOptions = res.filter(v => /^ZT/.test(v.pidType)).map(v => ({ value: v.pidType, label: v.pidType }))
      //       }
      //     })
      //   }
      // })
    ...
    getztLabel(val) {
      ...
      for (const j in comments) {
        const comj = comments[j]
        if (comj.pidType.slice(2) == val.label.slice(2) && comj.gyNameId == val.gyNameId && comj.gyObjId == val.gyObjId) {
          return val.label + '(' + comj.comment + ')'
        }
      }
    },
    handleFilter() {
          ...
          if (response.code === 0) {
            import('@/vendor/Export2Excel').then(excel => {
              let list = response.list
                ...
                //     if (comj.pidType.slice(2) == item.type.slice(2) && comj.gyNameId == item.snId.split('_')[2] && comj.gyObjId == item.snId.split('_')[0]) {
              ...
              const dataasd = {}
              // if (this.deviceTypeSelectValue !== '') {
              //   dataasd.gyObjId = this.deviceTypeSelectValue
              //   list = list.filter(v => v.smallObjId == dataasd.gyObjId)
              // }
              if (this.ztSelectValue !== '') {
                dataasd.type = this.ztSelectValue
                list = list.filter(v => v.type == dataasd.type)
              }
              if (this.deviceSelectValue !== '') {
                dataasd.farmName = this.deviceSelectValue
                list = list.filter(v => v.farmName == dataasd.farmName)
              }
              if (this.ztStateSelectValue !== '') {
                dataasd.ztState = this.ztStateSelectValue
                if (dataasd.ztState == 1) { list = list.filter(v => v.comment != '故障恢复') } else { list = list.filter(v => v.comment == '故障恢复') }
              }
              list = list.map(v => {
                const resd = v
