  ...
  <div>
    <el-dialog v-el-drag-dialog v-if="dialogStatus==='create'||dialogStatus==='update'||dialogStatus=='createChild'" :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" custom-class="user-management-diaCr">
      <el-form ref="dataForm" status-icon :rules="rules" :model="temp[dialogStatus]" size="mini" label-position="left" label-width="160px">
        ...
        <el-form-item label="上级设备" prop="pid">
          <!-- <span >{{temp[dialogStatus].pidName}}</span> -->
          <el-select v-model="temp[dialogStatus].pid" placeholder="上级设备">
            <el-option
              v-for="item in pidIdSelectOptions"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
        ...
        <el-form-item  v-for="(item,index) in deviceField" :key="index+'field'" :label="item.name" >
          <el-input v-model="temp[dialogStatus].ext[item.fieldImd]" style="width:calc(100% - 65px)"/>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        pid:[{ required: true, message: '上级设备不能为空', trigger: 'blur' }]
  ...
  watch: {
    'temp.create'(val) {
      this.$emit('update:deviceData',val)
    },
  ...
  methods: {
    ...
    getOtherSelectOptions() {
      ...
      this.getDeviceInfoOptions()
    },
    getDeviceInfoOptions() {
      mqttDeviceInfoList({ limit: '1',refProjectId:this.projectSelectValue }).then(response => {
        mqttDeviceInfoList({
          limit: response.page.totalCount.toString(),
          refProjectId:this.projectSelectValue
        }).then(response => {
          const res = response.page.list
          this.pidIdSelectOptions = res.map(item => ({ value: item.id, label: item.name }))
          this.pidIdSelectOptions = [{value:0,label:'无上级设备'},...this.pidIdSelectOptions]
        })
      })
    },
    // 选择框选项
