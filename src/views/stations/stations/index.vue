<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="snid" label="采集器ID" min-width="170" show-overflow-tooltip />
    ...
    getList() {
      ...
      mqttDeviceInfoList(data).then(response => {
        if (response.code === 0) {
          ...
          list = list.map(v=>({...v,ext:JSON.parse(v.ext)}))
          this.list = list
          
          if(this.sblxSelectOptionData.length>0) {
            const listTableColumn = this.sblxSelectOptionData.filter(v=>v.id==this.sblxSelectValue)
      ...
      this.$confirm('此操作将永久删除该设备, 是否继续?', '提示', {
        ...
        mqttDeviceInfoDelete(data).then(response => {
          if (response.code === 0) {
            ...
            this.getList()
            this.$refs.deviceInfo.getDeviceInfoOptions()
            this.$notify({
              title: 'Succeed',
              message: '删除成功',
              type: 'success',
