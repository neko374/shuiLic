<template>
  <div style="width:100%;height:100%;">
    ...
    >
      ...
      <el-table-column prop="name" label="汇流箱名称" min-width="100" show-overflow-tooltip />
      <el-table-column prop="pid" label="所属场站" min-width="100" show-overflow-tooltip>
        <template slot-scope="{row}">
          <span>{{ row.pid | selectOptionFilter(stationSelectOptions) }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="componentInfoEntity.snId" label="汇流箱ID" min-width="100" show-overflow-tooltip />
    ...
    <el-dialog v-if="dialogStatus==='create'||dialogStatus==='update'" :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" custom-class="user-management-diaCr">
      <el-form ref="dataForm" status-icon :rules="rules" :model="temp[dialogStatus]" size="mini" label-position="left" label-width="160px">
        <el-form-item label="汇流箱名称" prop="name">
          <el-input v-model="temp[dialogStatus].name" />
        </el-form-item>
        <el-form-item label="汇流箱ID" prop="snId">
          <el-input v-model="temp[dialogStatus].componentInfoEntity.snId" />
        </el-form-item>
        <el-form-item label="所属场站" prop="pid">
          <el-select v-model="temp[dialogStatus].pid" placeholder="所属场站">
            <el-option
