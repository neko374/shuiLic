      ...
      <el-form ref="dataForm" status-icon :rules="rules" :model="temp[dialogStatus]" size="mini" label-position="left" label-width="160px">
        ...
        <el-form-item label="所属设备" prop="pid">
          <el-select v-model="temp[dialogStatus].pid" placeholder="所属设备">
            <el-option
              v-for="item in deviceSelectOptions.filter(v=>v.value!=temp[dialogStatus].id)"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
...
import { getComponentSave, getComponentUpdate, getComponentDelete } from '@/api/mqtt/farmComponent'
  ...
  methods: {
      ...
      getFarmcfginfoList(data).then(response => {
        if (response.code === 0) {
          ...
          this.list = response.page.list.reduce((pre, cur) => {
            ...
            return pre
          }, []).map(v=>{for(let j in v) {if(v[j]=='null') v[j]='';return v}})
    ...
    getOtherSelectOptions() {
      ...
      getFarmcfginfoList(dataFcg).then(response => {
        ...
        getFarmcfginfoList(dataFcg).then(response => {
          const res = response.page.list
          this.deviceSelectOptions = res.map(item => ({ value: item.id, label: item.name }))
          this.deviceSelectOptions = [{value:0,label:'无'}].concat(this.deviceSelectOptions)
        })
      })
      const dataFcg1 = {
        limit: '1',
    ...
    createData({ ...data }) {
      this.$refs['dataForm'].validate(async (valid) => {
        if (valid) {
          ...
          data = { ...this.baseDataFilters,...data }
