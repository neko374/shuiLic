<template>
  <div style="width:100%;height:100%;">
    ...
    >
      ...
      <el-table-column prop="name" label="逆变器名称" min-width="100" show-overflow-tooltip />
      <el-table-column prop="pid" label="所属汇流箱名称" min-width="120" show-overflow-tooltip>
      ...
      <el-table-column prop="componentInfoEntity.snId" label="逆变器ID" min-width="100" show-overflow-tooltip />
    ...
    <el-dialog v-if="dialogStatus==='create'||dialogStatus==='update'" :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" custom-class="user-management-diaCr">
      <el-form ref="dataForm" status-icon :rules="rules" :model="temp[dialogStatus]" size="mini" label-position="left" label-width="160px">
        ...
        <el-form-item label="逆变器ID" prop="snId">
          <el-input v-model="temp[dialogStatus].componentInfoEntity.snId" />
        </el-form-item>
        <el-form-item label="所属汇流箱" prop="pid">
          <el-select v-model="temp[dialogStatus].pid" placeholder="所属汇流箱">
            <el-option
      ...
      getFarmcfginfoList(data).then(response => {
        if (response.code === 0) {
          ...
          this.list = response.page.list.reduce((pre, cur) => {
            ...
            },...cur, countryCascaderValue: [cur.cityId, cur.countyId] })
            return pre
          }, [])
