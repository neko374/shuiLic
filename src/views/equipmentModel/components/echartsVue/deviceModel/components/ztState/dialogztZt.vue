  ...
  <div>
    ...
    <el-dialog v-if="dialogStatus==='create'||dialogStatus==='update'" :title="textMap[dialogStatus]" :visible.sync="indialogFormVisible" custom-class="user-management-diaCrT">
      <el-form ref="dataForm" status-icon :rules="rules" :model="temp[dialogStatus]" size="mini" label-position="left" label-width="140px">
        ...
        <el-form-item label="状态信息" prop="comment">
...
import { mqttgybitZtvalList, mqttgybitZtvalSave, mqttgybitZtvalUpdate, mqttgybitZtvalDelete } from '@/api/mqtt/gybitZtval'
...
export default {
  ...
  methods: {
    getList() {
      ...
      mqttgybitZtvalList(data).then(response => {
        if (response.code === 0) {
    ...
    handleDelete(row) {
      this.$confirm('此操作将永久删除该状态, 是否继续?', '提示', {
        ...
        const data = [row.id]
        mqttgybitZtvalDelete(data).then(response => {
          if (response.code === 0) {
    ...
    createData({ ...data }) {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          mqttgybitZtvalSave(data).then(response => {
            if (response.code === 0) {
    ...
    updateData({ ...data }) {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          mqttgybitZtvalUpdate(data).then(response => {
            if (response.code === 0) {
              this.indialogFormVisible = false
              this.InitOptiList = true
              this.getList()
