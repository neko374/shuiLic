...
import { mqttgybitZtList, mqttgybitZtSave, mqttgybitZtUpdate, mqttgybitZtDelete, mqttgybitZtBatchSave, mqttgybitZtBatchUpdate } from '@/api/mqtt/gybitZt'
  ...
  methods: {
    ...
    importFileDemo(obj) {
      ...
      reader.onload = e => {
        ...
        // 10.在终端输出查看结果
        // console.log(j_data)
        mqttgybitZtList({ limit: '1', gyNameId: this.value.gyNameId, pidType: this.value.pidType }).then(response => {
          if (response.code !== 0) {
            this.$notify({
              title: 'Failed',
              message: response.msg,
          ...
          mqttgybitZtList({ limit: response.page.totalCount.toString(), gyNameId: this.value.gyNameId, pidType: this.value.pidType }).then(response => {
            if (response.code === 0) {
